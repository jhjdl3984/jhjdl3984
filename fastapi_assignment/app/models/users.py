...
# schemes=["bcrypt"] => bcrypt 방식으로 비밀번호 해시 생성
...
class UserModel:
    ...
    def get_hashed_password(password: str) -> str:
        """ 비밀번호 해시화 """
        return pwd_context.hash(password)   # 비밀번호를 bcrypt 방식으로 해시 생성해서 해시값으로 생성

    @staticmethod

    # plain_password => 로그인 시 입력한 비밀번호
    # hashed_password => 회원가입 시 입력한 비밀번호를 해시화 한 값
